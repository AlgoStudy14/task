 ...
 * 그런데 2차원 dp테이블을 만드는 경우에는 세로열을 채워나가는 부분 문제 구조와, 가로행을 채워나가는 부분 문제 구조가 2가지 존재할 수 있다.
 * (이 문제의 경우, 각 무게에 대하여 값을 갱신해나가는 과정 - 열에 대한 부분 문제/ 각 물건에 대하여 값을 갱신해나가는 과정 - 행에 대한 부분 문제로 나뉜다).
 * 이때, 이 문제에서 열에 대한 부분 문제를 구하는 과정은 앞에서 부터 계산한다면 최적 부분문제 구조를 성립할 수 없지만, 뒤에서 부터 계산한다면 최적 부분 문제 구조가 성립한다.
 * 따라서 해당 문제를 해결해주는 로직을 추가해준다면 1차원 dp테이블을 만들 수 있는 것이다.  
 */
