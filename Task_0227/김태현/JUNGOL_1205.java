...
/***
 * <문제 요약> 구해야 하는 것 : 가장 긴 스트레이트의 길이. 
 * 문제 유형 : 완전 탐색 
 * 요구 개념 : 인덱스 조작. 
 * <풀이법 요약> 
 * 1. 조커는 빼고, 나머지 숫자를 넣은뒤 정렬한다. 
 * 2. 각 숫자에 대하여 조커를 넣으며 최대 얼마나 길어지는지 탐색한다(얼마나 연결되는지).
 * -> 만일 다음 수가 같은 숫자라면 그냥 스킵한다. 
 * -> 만일 다음 수가 1차이가 난다면, 연결 길이를 + 1 한다. 
 * -> 만일 다음 수가 2이상 차이가 나는데 조커가 존재한다면, 조커 카드를 한 장 줄이고 연결 길이를 + 1 한다. 값도 + 1 한다. 
 * 3. 최종 최대 길이 값이 정답. 
 * -> 만일, 모든 카드가 조커로만 이루어져 있다면 조커의 개수가 정답.
 */
